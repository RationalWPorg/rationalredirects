...
# Build script for RationalRedirects WordPress plugin
# Creates a clean zip file for distribution

set -e

# Configuration
PLUGIN_SLUG="rationalredirects"
VERSION=$(grep -m1 "Version:" rationalredirects.php | sed 's/.*Version: *//' | tr -d '[:space:]')
BUILD_DIR="build"
