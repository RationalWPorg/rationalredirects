...
if ( ! defined( 'RATIONALREDIRECTS_MENU_VERSION' ) ) {
	define( 'RATIONALREDIRECTS_MENU_VERSION', '1.1.1' );
}

// Remote JSON URL for plugin data.
if ( ! defined( 'RATIONALREDIRECTS_PLUGINS_JSON_URL' ) ) {
	define( 'RATIONALREDIRECTS_PLUGINS_JSON_URL', 'https://rationalwp.com/plugins.json' );
}

// Cache duration in seconds (24 hours).
if ( ! defined( 'RATIONALREDIRECTS_PLUGINS_CACHE_DURATION' ) ) {
	define( 'RATIONALREDIRECTS_PLUGINS_CACHE_DURATION', DAY_IN_SECONDS );
}

if ( ! function_exists( 'rationalredirects_get_menu_icon' ) ) {
	/**
	 * Get the base64-encoded SVG icon for the menu.
	 *
	 * @return string Base64-encoded SVG data URI.
	 */
	function rationalredirects_get_menu_icon() {
...
if ( ! function_exists( 'rationalredirects_register_parent_menu' ) ) {
	/**
	 * Register the parent menu if it doesn't exist yet.
	 */
	function rationalredirects_register_parent_menu() {
		...
		add_menu_page(
			__( 'RationalWP', 'rationalredirects' ),
			__( 'RationalWP', 'rationalredirects' ),
			'manage_options',
			'rationalwp',
			'rationalredirects_render_parent_page',
			rationalredirects_get_menu_icon(),
			81 // After Settings (80).
		);
	}

	add_action( 'admin_menu', 'rationalredirects_register_parent_menu', 5 );
}

if ( ! function_exists( 'rationalredirects_fetch_remote_plugins' ) ) {
	/**
	 * Fetch plugin data from remote JSON URL.
	 *
	 * @return array|false Plugin data array on success, false on failure.
	 */
	function rationalredirects_fetch_remote_plugins() {
		// Add cache-busting parameter based on menu version.
		$url = add_query_arg( 'v', RATIONALREDIRECTS_MENU_VERSION, RATIONALREDIRECTS_PLUGINS_JSON_URL );
...
if ( ! function_exists( 'rationalredirects_get_available_plugins' ) ) {
	/**
	 ...
	 */
	function rationalredirects_get_available_plugins() {
		$cache_key = 'rationalredirects_plugins_list';
		...
		$remote_plugins = rationalredirects_fetch_remote_plugins();

		if ( false !== $remote_plugins ) {
			// Cache the successful response.
			set_transient( $cache_key, $remote_plugins, RATIONALREDIRECTS_PLUGINS_CACHE_DURATION );
...
if ( ! function_exists( 'rationalredirects_clear_plugins_cache' ) ) {
	/**
	 ...
	 */
	function rationalredirects_clear_plugins_cache() {
		return delete_transient( 'rationalredirects_plugins_list' );
	}
}

if ( ! function_exists( 'rationalredirects_render_parent_page' ) ) {
	/**
	 * Render the parent page content showing available plugins.
	 */
	function rationalredirects_render_parent_page() {
		$plugins   = rationalredirects_get_available_plugins();
		$installed = get_plugins();
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'RationalWP Plugins', 'rationalredirects' ); ?></h1>
			<p><?php esc_html_e( 'Practical tools for WordPress professionals.', 'rationalredirects' ); ?></p>

			<?php if ( empty( $plugins ) ) : ?>
				<div class="notice notice-warning">
					<p>
						<?php esc_html_e( 'Unable to load plugin information. Please check your internet connection and try again later.', 'rationalredirects' ); ?>
						<?php if ( current_user_can( 'manage_options' ) ) : ?>
							<br>
							<?php /* translators: %s: URL where plugin data is fetched from */ ?>
							<small><?php printf( esc_html__( 'Plugin data is fetched from %s', 'rationalredirects' ), '<code>' . esc_html( RATIONALREDIRECTS_PLUGINS_JSON_URL ) . '</code>' ); ?></small>
						<?php endif; ?>
					</p>
				</div>
			<?php else : ?>
			<div class="rationalredirects-plugins" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; margin-top: 20px;">
				<?php foreach ( $plugins as $plugin_key => $plugin ) : ?>
					...
					?>
					<div class="rationalredirects-plugin-card" style="background: #fff; border: 1px solid #c3c4c7; border-radius: 4px; padding: 20px;">
						<h3 style="margin-top: 0; margin-bottom: 10px;">
							<?php echo esc_html( $plugin['name'] ); ?>
							<?php if ( $is_active ) : ?>
								<span style="background: #00a32a; color: #fff; font-size: 11px; padding: 2px 8px; border-radius: 3px; margin-left: 8px; vertical-align: middle;"><?php esc_html_e( 'Active', 'rationalredirects' ); ?></span>
							<?php elseif ( $is_installed ) : ?>
								<span style="background: #dba617; color: #fff; font-size: 11px; padding: 2px 8px; border-radius: 3px; margin-left: 8px; vertical-align: middle;"><?php esc_html_e( 'Inactive', 'rationalredirects' ); ?></span>
							<?php endif; ?>
						</h3>
						<p style="color: #646970; margin-bottom: 15px;"><?php echo esc_html( $plugin['description'] ); ?></p>
						<div class="rationalredirects-plugin-actions">
							<?php if ( $is_active ) : ?>
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $plugin['menu_slug'] ) ); ?>" class="button button-primary"><?php esc_html_e( 'Settings', 'rationalredirects' ); ?></a>
							<?php elseif ( $is_installed ) : ?>
								<a href="<?php echo esc_url( wp_nonce_url( admin_url( 'plugins.php?action=activate&plugin=' . urlencode( $plugin['file'] ) ), 'activate-plugin_' . $plugin['file'] ) ); ?>" class="button button-primary"><?php esc_html_e( 'Activate', 'rationalredirects' ); ?></a>
							<?php else : ?>
								<a href="<?php echo esc_url( $plugin['url'] ); ?>" class="button" target="_blank"><?php esc_html_e( 'Learn More', 'rationalredirects' ); ?></a>
							<?php endif; ?>
						</div>
					</div>
				<?php endforeach; ?>
			 ...
			 */
			do_action( 'rationalredirects_parent_page_content' );
			?>
		</div>
		<?php
	}
